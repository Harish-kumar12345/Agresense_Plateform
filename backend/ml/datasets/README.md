...
| **`dbt_agriculture_schemes.csv`** | 20 verified Central and State government welfare schemes & subsidies (PM-KISAN, PMFBY, KCC, SMAM, PKVY, PM-KUSUM, UP Paradarshi, MP Bhavantar, Haryana Bhavantar Bharpai, etc.). | myScheme / DBT Agriculture (`myscheme.gov.in`) | GODL-India |
| **`icar_kvk_master_directory.csv`** | 794 verified ICAR Krishi Vigyan Kendras (KVKs) across all 36 States/UTs with host institutions, pincodes, phone numbers, and coordinates. | ICAR Krishi Vigyan Kendra Portal (`kvk.icar.gov.in`) | Open Access |
| **`cibrc_approved_agrochemicals_phi.csv`** | 25 CIBRC statutorily approved agrochemical formulations, target crops, pests, doses per ha/acre, and Pre-Harvest Intervals (PHI). | Central Insecticide Board & Registration Committee (`cibrc.gov.in`) | Statutory Public Guidance |
| **`des_agri_census_district_acreage.csv`** | 733 districts aggregated from DES Agriculture Census with crop sowing acreage, production, and yield benchmarks. | Directorate of Economics & Statistics (`aps.dac.gov.in`) | GODL-India |
