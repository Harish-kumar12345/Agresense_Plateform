...
export const SmartAlertsCenter: React.FC<SmartAlertsCenterProps> = ({
  ...
  const getSeverityBadge = (severity: AlertSeverity) => {
    switch (severity) {
      case 'Critical':
        return (
          <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-rose-500/15 text-rose-300 border border-rose-500/30 flex items-center gap-1">
            <ShieldAlert className="w-3.5 h-3.5 text-rose-400" /> CRITICAL
          </span>
        );
      case 'High':
        return (
      ...
      case 'Warning':
        return (
          <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-yellow-500/15 text-yellow-300 border border-yellow-500/30 flex items-center gap-1">
            <AlertTriangle className="w-3.5 h-3.5 text-yellow-400" /> WARNING
          </span>
        );
      default:
        return (
          <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-sky-500/15 text-sky-300 border border-sky-500/30 flex items-center gap-1">
            <Info className="w-3.5 h-3.5 text-sky-400" /> INFO
          </span>
        );
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-end bg-slate-950/70 backdrop-blur-md">
      ...
      <motion.div
        ...
        className="relative w-full max-w-2xl h-full bg-slate-950 border-l border-white/10 shadow-2xl flex flex-col overflow-hidden z-10"
      >
        {/* Drawer Header */}
        <div className="p-5 border-b border-white/10 bg-slate-900/90 backdrop-blur-xl flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
            {unreadCount > 0 && (
              <button
                ...
                className="px-3 py-1.5 text-xs font-semibold rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white border border-white/10 transition-colors flex items-center gap-1.5 cursor-pointer"
              >
        ...
        {/* Filter Bar */}
        <div className="px-5 py-3 border-b border-white/5 bg-slate-900/50 space-y-2.5 shrink-0">
          {/* Severity Filters */}
              ...
              <button
                ...
                className={`px-3 py-1 rounded-lg border text-xs font-semibold transition-all cursor-pointer ${
                  severityFilter === sev
                    ...
                    : 'bg-slate-800/80 text-slate-400 border-white/5 hover:bg-slate-800 hover:text-white'
                }`}
              >
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3.5">
          {loading ? (
            <div className="py-24 text-center text-slate-400 space-y-3">
              ...
              <p className="text-sm font-semibold">Evaluating live telemetry matrices...</p>
            </div>
          ) : alerts.length === 0 ? (
            <div className="py-24 text-center text-slate-500 space-y-3 border-2 border-dashed border-slate-800/80 rounded-3xl p-6">
              <CheckCheck className="w-12 h-12 text-emerald-500/60 mx-auto" />
              <h3 className="text-lg font-bold text-slate-200 font-display">All Parameters Optimal</h3>
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-2xl border transition-all ${
                    isUnread
                      ? 'bg-slate-900/90 border-slate-700 shadow-lg shadow-black/40 ring-1 ring-emerald-500/30'
                      : 'bg-slate-900/50 border-white/5 opacity-85 hover:opacity-100'
                  }`}
                >
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[11px] font-semibold px-2 py-0.5 rounded-md bg-slate-950 text-slate-300 border border-white/10">
